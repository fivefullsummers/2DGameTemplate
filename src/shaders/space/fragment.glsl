...
// Larger NEBULA_SCALE = bigger, more expansive clouds (was 60.0; ~3x for mobile/portrait)
const float NEBULA_SCALE = 180.0;
const float NEBULA_STRENGTH = 0.08;
...
// Number of pixel blocks along the shorter axis (lower = chunkier)
const float PIXEL_GRID = 80.0;
// When uMouseActive=1, blend rays toward mouse (stars appear attracted to cursor)
const float MOUSE_ATTRACT_STRENGTH = 0.45;
const float MOUSE_ATTRACT_RADIUS = 0.55;

void main() {
    vec2 fragCoord = vUV * uResolution;

    // i_pos, f_pos pixelation: sample at block center so each block shares one color
    ...
    // Attract rays toward mouse when press/drag: bend ray toward the ray through uMouse
    vec2 uvNorm = sampleCoord / uResolution;
    float distToMouse = length(uvNorm - uMouse);
    float pull = uMouseActive * (1.0 - smoothstep(0.0, MOUSE_ATTRACT_RADIUS, distToMouse));
    vec3 rayDirToMouse = getRayDirection(uMouse * uResolution, movementDirection);
    rayDirection = normalize(mix(rayDirection, rayDirToMouse, pull * MOUSE_ATTRACT_STRENGTH));
    vec3 directionSign = sign(rayDirection);
